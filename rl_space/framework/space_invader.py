...
class SpaceInvaderGame:
    ...
    def step(self, action):
        ...
        for i in range(n_lasers):
            if self.lasers[i].y > HEIGHT:
                ...
                self.enemies[i].reloading = False
                self.lasers[i].x = (
                    self.enemies[i].x
                    + self.enemies[i].width / 2
                    - self.lasers[i].width / 2
    ...
    def snap_data(self):
        ...
        for enemy in self.enemies:
            enemy_x = enemy.x
            enemy_y = enemy.y
            enemy_reloading = enemy.reloading
    ...
    def save_dataset(self):
        ...
        df = df.reset_index()
        df["img_path"] = df["index"].apply(lambda x: f"{dataset_images}/{x}.jpg")
        paths = df["img_path"].values.tolist()

        for i in range(len(self.dataset)):
            path = paths[i]
            img = self.dataset[i][3]
